...
export default function Navbar() {
  ...
  // Removed duplicate handleLinkClick declaration
    ...
    <header className="bg-white backdrop-blur-md fixed top-0 w-full z-50 border-b border-white">
      <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
        ...
        <nav className="hidden lg:flex flex-1 justify-center space-x-8">
          ...
          {/* Dropdown Expertises (hover) */}
          <div
            className="relative"
            onMouseEnter={() => setDropdownOpen(true)}
            onMouseLeave={() => setDropdownOpen(false)}
          >
            <button className={`${navLinkClass('')} inline-flex items-center focus:outline-none`}>
              {t('navbar.expertises')}
              <ChevronDownIcon
        ...
        <div className="hidden lg:flex items-center space-x-4">
         ...
         <MagnifyingGlassIcon className="h-5 w-5" />
          ...
          <kbd className="hidden xl:inline px-2 py-1 text-xs font-mono text-gray-500 border border-gray-300 rounded">
           Ctrl + K
          </kbd>
        </button>
